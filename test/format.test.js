...
test('AIO stream has parseable filename, language and size', () => {
  ...
  assert.equal(stream.behaviorHints.filename, 'Titolo [Test] (2025).1080p.mp4');
  assert.match(stream.description, /🌐 Source: Test/);
...
test('AIO stream exposes the parser fields used by AIOStreams', () => {
  ...
  assert.equal(stream.title, 'Serie [Source]');
  assert.equal(stream.behaviorHints.filename, 'Serie [Source].S01E02.mp4');
  assert.equal(stream.behaviorHints.videoSize, 1234);
  assert.match(stream.description, /^Serie \[Source\]\.S01E02\.mp4\n/);
});
...
test('AIO filename does not duplicate an episode already in the title', () => {
  ...
  assert.equal(stream.behaviorHints.filename, 'Dragon Ball GT S01E01 [Test].mp4');
});
